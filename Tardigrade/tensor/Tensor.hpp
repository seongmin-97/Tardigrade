...
public:
    Storage(size_t size) { m_data = std::make_shared<std::vector<double>>(size, 0.0); }

    Storage() { m_data = std::make_shared<std::vector<double>>(); }

    double *GetData() { return m_data->data(); }

    const double *GetData() const { return m_data->data(); }

    size_t GetSize() const { return m_data->size(); }

    void Resize(size_t newSize) { m_data->resize(newSize, 0.0); }
...
public:
    TensorImpl(const Shape &shape, bool requiresGrad = false)
        : m_shape(shape), m_strides(calculateStrides(shape)), m_storage(0), m_gradNode(nullptr), m_grad(nullptr),
...
public:
    ...
    operator double() const
    {
        if (size() != 1)
        {
            throw std::runtime_error(
                "Implicit conversion to double is only supported for Tensors with exactly 1 element.");
        }
        return data()[0];
    }
    ...
    Tensor operator[](int index) const { return select(0, index); }

    /** @brief Multidimensional variadic indexing operator returning a reference to a scalar value: t(i, j, k) */
    template <typename... Args> double &operator()(Args... indices)
    {
    ...
    template <typename... Args> double operator()(Args... indices) const
    {
        std::vector<int> idx = {static_cast<int>(indices)...};
        return m_impl->m_storage[calculateIndex(idx)];
    }
 ...
 *  Now autograd-tracked via Im2colNode (backward = col2im). */
...
Tensor col2im(const Tensor &col, const Shape &inputShape, int kernelH, int kernelW, int strideH, int strideW, int padH,
              int padW);

/** @brief Axis-wise maximum reduction: Y_i = max_{j} X_{...,j,...}.
 *  Autograd-tracked via ReduceMaxNode (backward = scatter-add to argmax positions). */
Tensor reduce_max(const Tensor &X, int axis, bool keepDims = false);

/** @brief 2D Cross-Correlation as a named composition of primitives (im2col + reshape + matmul + reshape + permute).
 *  No new Node created — uses the existing autograd graph transparently.
 *  Input: [N, C_in, H, W], Kernel: [C_out, C_in, Kh, Kw] */
Tensor convolve(const Tensor &input, const Tensor &kernel, int stride = 1, int padding = 0);
