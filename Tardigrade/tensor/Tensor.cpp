...
double &Tensor::operator[](size_t index) { return m_impl->m_storage[index]; }

const double &Tensor::operator[](size_t index) const { return m_impl->m_storage[index]; }
...
Tensor Tensor::row(int index) const { return slice(0, index, index + 1); }

void Tensor::setRow(int index, const Tensor &src) { setSlice(0, index, src); }

Tensor Tensor::col(int index) const { return slice(1, index, index + 1); }

void Tensor::setCol(int index, const Tensor &src) { setSlice(1, index, src); }
...
Tensor mul(const Tensor &A, const Tensor &B)
{
    if (A.shape() != B.shape())
    {
        throw std::runtime_error("Shape mismatch for elementwise mul.");
    }

    Tensor C(A.shape(), A.requiresGrad() || B.requiresGrad());
    C.asVector() = A.asVector().cwiseProduct(B.asVector());
...
Tensor div(const Tensor &A, const Tensor &B)
{
    if (A.shape() != B.shape())
    {
        throw std::runtime_error("Shape mismatch for elementwise div.");
    }

    Tensor C(A.shape(), A.requiresGrad() || B.requiresGrad());
    C.asVector() = A.asVector().cwiseQuotient(B.asVector());
...
Tensor sum(const Tensor &X, int axis)
{
    ...
    if (X.rank() == 2)
    {
        ...
        if (axis == 0)
        {
            Tensor Y({1, cols}, X.requiresGrad());
            Y.asMatrix() = X.asMatrix().colwise().sum();
        ...
        {
            Tensor Y({rows, 1}, X.requiresGrad());
            Y.asMatrix() = X.asMatrix().rowwise().sum();
    ...
    throw std::runtime_error("Unsupported axis for sum operation.");
}
...
Tensor Tensor::transpose() const { return tardigrade::transpose(*this); }
...
Tensor operator*(double scalar, const Tensor &rhs) { return rhs * scalar; }

Tensor operator/(const Tensor &lhs, const Tensor &rhs) { return div(lhs, rhs); }

Tensor operator/(const Tensor &lhs, double scalar) { return div(lhs, scalar); }
} // namespace tardigrade
